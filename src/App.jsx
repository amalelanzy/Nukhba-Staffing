...
// ==========================================
// 1. القواميس والترجمات (Translations)
// ==========================================
const translations = {
  ar: {
    nav: { home: 'الرئيسية', features: 'المميزات', nations: 'الجنسيات', pricing: 'الباقات', order: 'اطلب الآن' },
    hero: [
      { title: "نُخبة الكوادر.. فخامة تليق بمنزلكِ", subtitle: "ننتقي لكِ الصفوة لضمان جودة الحياة وراحة عائلتكِ الدائمة برؤية ملكية.", tag: "نُخبة الاختيار" },
      { title: "وصول سريع.. التزام بالوقت", subtitle: "نلتزم بأقل مدة وصول في المملكة عبر إجراءات رسمية وموثقة من مساند.", tag: "السرعة الذهبية" },
      { title: "حقوقكِ محفوظة بعقود رسمية", subtitle: "عقود رسمية تضمن لكِ الأمان والاحترافية الكاملة في كل خطوة استقدام.", tag: "ثقة مطلقة" }
    ],
    stats: [
      { label: "عائلات نُخبة سعيدة", value: "+1,500" },
      { label: "عقود مضمونة", value: "100%" },
      { label: "تغطية شاملة", value: "المملكة" },
      { label: "سرعة استقدام", value: "قياسية" }
    ],
    steps: {
  ...
  en: {
    ...
    hero: [
      { title: "Elite Staff.. Luxury for Your Home", subtitle: "We select the best to ensure quality of life and permanent comfort for your family.", tag: "Elite Selection" },
      { title: "Rapid Arrival.. Timely Commitment", subtitle: "Shortest arrival time in KSA through documented Musaned procedures.", tag: "Golden Speed" },
      { title: "Your Rights are Officially Secured", subtitle: "Official contracts ensure safety and full professionalism at every step.", tag: "Absolute Trust" }
    ],
    stats: [
    ...
    <div className="max-w-7xl mx-auto px-4 md:px-10 flex justify-between items-center">
      ...
      <div className={`flex items-center gap-3 ${lang === 'ar' ? 'text-right' : 'text-left'}`}>
        ...
        <div className={`flex flex-col leading-none ${lang === 'ar' ? 'items-end' : 'items-start'}`}>
          ...
          {/* تم ضبط المحاذاة والتباعد للكلمة الصفراء */}
...
const App = () => {
  ...
  return (
    <div className={`min-h-screen bg-[#FDFBF7] font-sans selection:bg-amber-200 overflow-x-hidden ${lang === 'ar' ? 'text-right' : 'text-left'}`} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      {/* Hero Section */}
      <section className="relative h-screen min-h-[500px] w-full overflow-hidden bg-slate-900 flex items-center justify-center">
        {slides.map((slide, index) => (
          <div key={index} className={`absolute inset-0 transition-opacity duration-[1500ms] ease-in-out ${index === currentSlide ? 'opacity-100 z-10' : 'opacity-0 z-0 pointer-events-none'}`}>
            ...
            <div className="relative h-full max-w-7xl mx-auto px-6 flex flex-col items-center justify-center pt-16 pb-12 z-20 text-center">
              <div className="max-w-4xl space-y-5 md:space-y-7">
                <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-amber-600/90 text-white font-bold text-[9px] md:text-[10px] uppercase tracking-widest shadow-2xl mx-auto backdrop-blur-sm" style={{ animation: index === currentSlide ? 'fadeInUp 0.8s ease' : '' }}>
                  <Award className="w-3.5 h-3.5" /> {slide.tag}
                </div>
                <h1 className={`text-3xl md:text-5xl lg:text-[4.5rem] font-black text-white leading-[1.2] tracking-tighter drop-shadow-2xl px-4 ${lang === 'en' ? 'lg:text-[3.8rem]' : ''}`} style={{ animation: index === currentSlide ? 'fadeInUp 0.8s ease 0.2s backwards' : '' }}>
                  {slide.title}
                </h1>
                <p className="text-sm md:text-lg lg:text-xl text-white/90 font-medium max-w-2xl mx-auto leading-relaxed px-4" style={{ animation: index === currentSlide ? 'fadeInUp 0.8s ease 0.4s backwards' : '' }}>
                  {slide.subtitle}
                </p>
                <div className="flex flex-col sm:flex-row gap-4 md:gap-6 justify-center pt-6" style={{ animation: index === currentSlide ? 'fadeInUp 0.8s ease 0.6s backwards' : '' }}>
                  <button onClick={() => handleOrderClick()} className="bg-amber-600 text-white px-8 md:px-12 py-4 md:py-5 rounded-full font-black text-base md:text-xl hover:bg-amber-700 shadow-xl transition-all transform hover:scale-105 active:scale-95">
                    {lang === 'ar' ? 'احجزي نُخبتكِ الآن' : 'Book Now'}
                  </button>
                  <a href={`tel:${phoneNumber}`} className="bg-white/10 backdrop-blur-md text-white border-2 border-white/20 px-8 md:px-12 py-4 md:py-5 rounded-full font-black text-base md:text-xl hover:bg-white/20 transition-all flex items-center justify-center gap-3">
                    <Phone className="w-5 h-5" /> {lang === 'ar' ? 'إتصال سريع' : 'Quick Call'}
                  </a>
                </div>
              </div>
            </div>
        ...
        <button onClick={prevSlide} className={`absolute ${lang === 'ar' ? 'right-4 md:right-8' : 'left-4 md:left-8'} top-1/2 -translate-y-1/2 z-50 w-10 h-10 md:w-12 md:h-12 flex items-center justify-center rounded-full border border-white/10 bg-black/10 hover:bg-amber-600 text-white transition-all backdrop-blur-md hover:scale-110 active:scale-90 pointer-events-auto`}>
          {lang === 'ar' ? <ChevronRight size={20} /> : <ChevronLeft size={20} />}
        </button>
        <button onClick={nextSlide} className={`absolute ${lang === 'ar' ? 'left-4 md:left-8' : 'right-4 md:right-8'} top-1/2 -translate-y-1/2 z-50 w-10 h-10 md:w-12 md:h-12 flex items-center justify-center rounded-full border border-white/10 bg-black/10 hover:bg-amber-600 text-white transition-all backdrop-blur-md hover:scale-110 active:scale-90 pointer-events-auto`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12">
            {[Search, ClipboardCheck, Truck].map((Icon, i) => (
              <div key={i} className={`relative flex flex-col items-center p-10 md:p-14 rounded-[3.5rem] bg-white transition-all duration-700 reveal delay-${i+1} ${stepsVisible ? 'visible' : ''} border border-transparent hover:border-amber-100 group shadow-sm hover:shadow-2xl animate-float`} style={{ animationDelay: `${i * 0.5}s` }}>
                {/* تم تعديل الشفافية واللون هنا لتكون واضحة جداً */}
                <div className={`absolute top-6 ${lang === 'ar' ? 'right-8' : 'left-8'} text-slate-200/90 font-black text-7xl md:text-8xl transition-colors group-hover:text-amber-200/50 -z-0`}>0{i+1}</div>
      ...
      {/* Pricing Section - تم ضبط البادينج العمودي */}
      <section ref={pricingRef} id="pricing" className="py-24 md:py-32 bg-[#FDFBF7] relative z-40 px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10 max-w-7xl mx-auto">
          {packages.map((pkg, i) => (
            <div key={i} className={`group relative flex flex-col p-10 md:p-12 rounded-[3.5rem] md:rounded-[4rem] transition-all duration-700 reveal delay-${i+1} ${pricingVisible ? 'visible' : ''} ${pkg.popular ? 'bg-slate-900 text-white scale-100 lg:scale-105 shadow-2xl' : 'bg-white text-slate-900 border border-slate-100 shadow-sm hover:shadow-xl'}`}>
              {pkg.popular && <div className={`absolute top-0 ${lang === 'ar' ? 'right-1/2 translate-x-1/2' : 'left-1/2 -translate-x-1/2'} -translate-y-1/2 bg-amber-600 text-white px-7 py-2 rounded-full text-[9px] font-black uppercase tracking-widest shadow-xl animate-pulse`}>{t.pricing.popular}</div>}
              <h3 className={`text-xl md:text-2xl font-bold mb-6 ${pkg.popular ? 'text-amber-400' : 'text-amber-600'}`}>{pkg.title}</h3>
              <div className="flex items-baseline gap-2 mb-8 text-right"><span className="text-3xl md:text-5xl font-black tracking-tighter tabular-nums">{pkg.price}</span><span className={`text-sm md:text-base font-bold ${pkg.popular ? 'text-slate-400' : 'text-slate-500'}`}>{t.pricing.currency}</span></div>
              <div className={`h-px w-full mb-8 ${pkg.popular ? 'bg-white/10' : 'bg-amber-100'}`}></div>
              <ul className="space-y-6 mb-12 flex-grow">
                {pkg.features.map((f, idx) => (
                  <li key={idx} className="flex items-center gap-3 font-bold text-sm transition-transform group-hover:translate-x-[-5px]"><CheckCircle2 className={`w-5 h-5 md:w-6 md:h-6 shrink-0 ${pkg.popular ? 'text-amber-500' : 'text-amber-600'}`} /> {f}</li>
                ))}
              </ul>
              {/* تم زيادة البادينج هنا ليكون فخماً كما طلبتِ */}
              <button onClick={() => handleOrderClick(pkg)} className={`w-full py-5 md:py-6 px-10 rounded-full font-black text-lg md:text-xl transition-all transform hover:scale-105 active:scale-95 shadow-lg ${pkg.popular ? 'bg-amber-600 text-white hover:bg-amber-700' : 'bg-slate-900 text-white hover:bg-amber-600'}`}>
                {t.pricing.book}
              </button>
            </div>
          ))}
      ...
      {/* Footer */}
      <footer className="bg-white py-20 border-t border-slate-100 relative z-40 text-center px-6">
        <div className="max-w-7xl mx-auto space-y-10">
            <div className="flex flex-col items-center gap-4">
                <div className="w-12 h-12 bg-amber-600 rounded-[1.5rem] flex items-center justify-center shadow-2xl">
                  <Award className="text-white w-7 h-7 fill-white" />
                </div>
                <span className="text-2xl md:text-3xl font-black text-slate-900 tracking-tighter italic">{lang === 'ar' ? 'نُخبة الكوادر' : 'Nukhba Staffing'}</span>
            </div>
            <p className="text-slate-400 font-bold text-base md:text-xl max-w-2xl mx-auto leading-relaxed text-center">{t.footer.desc}</p>
            <div className="pt-10 border-t border-slate-50 flex flex-col md:flex-row justify-between items-center gap-6 text-center">
              ...
              <div className="flex gap-8 text-slate-400 text-[10px] md:text-sm font-black uppercase tracking-widest">
      ...
      {/* Modal Form */}
      {showModal && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 md:p-6 bg-slate-900/98 backdrop-blur-xl animate-in fade-in duration-500">
          <div className="bg-white w-full max-md rounded-[2.5rem] p-8 md:p-14 shadow-2xl relative overflow-y-auto max-h-[90vh] animate-in zoom-in duration-500 text-right">
            <button onClick={() => setShowModal(false)} className={`absolute top-5 ${lang === 'ar' ? 'left-5' : 'right-5'} text-slate-400 hover:text-slate-900 p-2 hover:rotate-90 transition-all`}><X size={20} /></button>
            <div className="space-y-8">
                <div className="text-center space-y-3">
                  <h3 className="text-xl md:text-2xl font-black text-slate-900 tracking-tighter">{t.modal.title}</h3>
                  <p className="text-slate-500 text-sm font-bold italic">{t.modal.package}: <span className="text-amber-600">{selectedPackage?.title}</span></p>
                </div>
                <form onSubmit={sendToWhatsApp} className="space-y-5">
                    <div className="space-y-1.5">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t.modal.name}</label>
                        <div className="relative">
                            <User className={`absolute ${lang === 'ar' ? 'right-4' : 'left-4'} top-1/2 -translate-y-1/2 text-slate-300 w-5 h-5`} />
                            <input required type="text" placeholder={t.modal.namePlaceholder} className={`w-full bg-[#FDFBF7] border-2 border-transparent rounded-2xl py-3.5 md:py-4 ${lang === 'ar' ? 'pr-12' : 'pl-12'} focus:border-amber-600 focus:bg-white outline-none font-bold text-sm md:text-base transition-all`} value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} />
                        </div>
                    </div>
                    <div className="space-y-1.5">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t.modal.city}</label>
                        <div className="relative">
                            <MapPin className={`absolute ${lang === 'ar' ? 'right-4' : 'left-4'} top-1/2 -translate-y-1/2 text-slate-300 w-5 h-5`} />
                            <input required type="text" placeholder={t.modal.cityPlaceholder} className={`w-full bg-[#FDFBF7] border-2 border-transparent rounded-2xl py-3.5 md:py-4 ${lang === 'ar' ? 'pr-12' : 'pl-12'} focus:border-amber-600 focus:bg-white outline-none font-bold text-sm md:text-base transition-all`} value={formData.city} onChange={(e) => setFormData({...formData, city: e.target.value})} />
                        </div>
                    </div>
                    <button type="submit" className="w-full bg-slate-900 text-white py-5 md:py-6 rounded-full font-black text-lg md:text-xl hover:bg-amber-600 shadow-2xl transition-all flex items-center justify-center gap-4 md:gap-6 mt-6 transform hover:-translate-y-2 active:scale-95">{t.modal.confirm} <Send className={`w-4 h-4 ${lang === 'ar' ? 'rotate-180' : ''}`} /></button>
                </form>
            </div>
          </div>
        </div>
